...
from .model_clients.comfyui_client import comfyui_client
...
IMAGE_KEYWORDS = (
    "draw", "generate an image", "generate a image", "picture of",
    "illustration", "logo", "photo of", "image of", "render a",
    "create an image", "make an image", "make a picture", "design a poster",
...
async def route_image(prompt: str, model: str | None = None):
    if model == "disabled" or (
        not settings.enable_local_image_generation
        and not settings.enable_remote_image_generation
    ):
        raise ModelUnavailableError(
            "Image generation is available only in OmniGen's local GPU mode. "
            "This free hosted demo supports chat and document RAG."
        )
    if model == "local":
        if not settings.enable_local_image_generation:
            raise ModelUnavailableError("Local GPU image generation is not available in this hosted deployment.")
        return await comfyui_client.generate_image(prompt)
    if not settings.enable_remote_image_generation:
        raise ModelUnavailableError("Remote image generation is disabled for this deployment.")
