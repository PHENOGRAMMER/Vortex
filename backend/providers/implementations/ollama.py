...
class OllamaProvider(BaseProvider):
    ...
    async def embeddings(
        ...
        print("OllamaProvider.embeddings() called.")

        print("="*70)
        print("Argument model: ", repr(model))
        print("Default Model: ", repr(settings.DEFAULT_EMBEDDING_MODEL))

        model = model or settings.DEFAULT_EMBEDDING_MODEL

        print("Final Model: ", repr(model))
        print("="*70)

        print(f"Requesting embeddings for {len(input) if isinstance(input, list) else 1} text(s).")

        vectors = await self.client.embeddings(
            text=input,
            model=model,
        )
